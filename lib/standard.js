...
// Tests
joe.suite('standard example', function (suite, test) {
	...
	suite('tests', function (suite) {
		...
		suite('before and after', function (suite, test) {
			...
			test('test 3', {before: before, after: after}, function () {
				checks.push('test 3')
				assert.deepEqual(checks, [
					'before - test 1 - part 1',
					'before - test 1 - part 2',
		...
		suite('deliberate-test-failure ignored', function (suite, test) {
			var err1 = new Error('deliberate error 1')
			var err2 = new Error('deliberate error 2')
